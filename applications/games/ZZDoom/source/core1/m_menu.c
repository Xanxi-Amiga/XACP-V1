...
void M_SaveSelect(int choice)
{
    /* ZZDoom: no keyboard input - use auto name "Save N" and save immediately */
    char autoname[SAVESTRINGSIZE];
    M_snprintf(autoname, sizeof(autoname), "Save %d", choice + 1);
    M_StringCopy(savegamestrings[choice], autoname, SAVESTRINGSIZE);
    saveStringEnter = 0;
    M_DoSave(choice);
}
