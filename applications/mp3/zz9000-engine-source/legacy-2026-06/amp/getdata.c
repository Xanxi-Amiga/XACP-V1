 ...
 */
int decode_scalefactors(struct Amp *itsAmp, struct SIDE_INFO *info,struct AUDIO_HEADER *header,int gr,int ch)
{
int sfb,window;
int slen1,slen2;
int i1,i2,i=0;
	 ...
	 */
		...
		if (info->window_switching_flag[gr][ch] && info->block_type[gr][ch]==2) {
			if (info->mixed_block_flag[gr][ch]) {
				for (sfb=0;sfb<8;sfb++) {
					itsAmp->scalefac_l[gr][ch][sfb]=getbits(itsAmp, slen1);	
					i+=slen1;
				}
				for (sfb=3;sfb<6;sfb++) {
					for (window=0;window<3;window++)
						itsAmp->scalefac_s[gr][ch][sfb][window]=getbits(itsAmp, slen1);
					i+=i1;
				}
				for (;sfb<12;sfb++) {
					for (window=0;window<3;window++)
						itsAmp->scalefac_s[gr][ch][sfb][window]=getbits(itsAmp, slen2);
					i+=i2;
				}
			...
			for (window=0;window<3;window++) 
				itsAmp->scalefac_s[gr][ch][12][window]=0;
		} else { /* block_type!=2 */
			if ( !info->scfsi[ch][0] || !gr )
				...
				itsAmp->scalefac_l[1][ch][sfb]=itsAmp->scalefac_l[0][ch][sfb];
			}	
			...
			if ( !info->scfsi[ch][2] || !gr )
				for (sfb=11;sfb<16;sfb++) {
					itsAmp->scalefac_l[gr][ch][sfb]=getbits(itsAmp, slen2);
					i+=slen2;
				}
		...
		if ( (header->mode_extension==1 || header->mode_extension==3) && ch==1) { /* right ch... */
			...
			itsAmp->intensity_scale=info->scalefac_compress[0][1]&1;
			spooky_index=1;
		...
		if (!info->window_switching_flag[0][ch] || (info->window_switching_flag[0][ch] && info->block_type[0][ch]!=2)) {
			...
			for (j=1;j<=4;j++) 
				for (k=0;k<nr_of_sfb[j];k++) {
					itsAmp->scalefac_l[0][ch][sfb]=getbits(itsAmp, slen[j]);
					i+=slen[j];
					if (ch) itsAmp->is_max[sfb]=(1<<slen[j])-1;
					sfb++;
				}
			...
			if (!info->mixed_block_flag[0][ch]) {
				...
				for (j=1;j<=4;j++)
					for (k=0;k<nr_of_sfb[j];k+=3) {
						 ...
						 */
						itsAmp->scalefac_s[0][ch][sfb][0]=getbits(itsAmp, slen[j]);
						itsAmp->scalefac_s[0][ch][sfb][1]=getbits(itsAmp, slen[j]);
						itsAmp->scalefac_s[0][ch][sfb][2]=getbits(itsAmp, slen[j]);
						i+=3*slen[j];
						if (ch) itsAmp->is_max[sfb+6]=(1<<slen[j])-1;
						sfb++;
					}
			} else {
				/* what we do here is:
				...
				sfb=0;
				for (k=0;k<6;k++) {
					itsAmp->scalefac_l[0][ch][sfb]=getbits(itsAmp, slen[1]);
					i+=slen[j];
					if (ch) itsAmp->is_max[sfb]=(1<<slen[1])-1;
					sfb++;
				}
				nr_of_sfb[1]-=6;
				sfb=3;
