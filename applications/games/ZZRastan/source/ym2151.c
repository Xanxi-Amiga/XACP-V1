...
/* FBNeo savestate removed for bare metal (no synthesis code touched). */
...
int YM2151Init(int num, int chipbase, int clock, int rate, void (*timer_cb)(INT32, INT32, double))
{
	...
	YMNumChips = num;
	    ...
	    {   /* ZZRastan: one chip only, static storage - no allocator on bare metal. */
        static YM2151 g_ym2151_storage[1];
        YMPSG = g_ym2151_storage;
    }
	if (YMPSG == NULL)
		return 1;
