...
static void convert_lut_p96(const u16 *src, u32 *dst, u32 stride_px, u32 loffs, u32 lines){
    u32 yy,xx;
    const u32 black=g_lut565[0];   /* 0xFF000000 noir opaque */
    /* PicoDrive positionne DEJA l'image active a FB3A[loffs..loffs+lines-1]
       (draw.c:1741 : loffs=8/lines=224 NTSC -> bandes 8 haut + 8 bas symetriques ;
       loffs=0/lines=240 en mode 240). On copie donc 1:1 (SURTOUT pas de re-offset,
       sinon double decalage) et on force les bordures en noir (elles contiennent
       le sentinelle diagnostic, pas de l'image). */
    for(yy=0;yy<FB3A_H;yy++){
        ...
        if(yy<loffs || yy>=loffs+lines){
            ...
            for(xx=0;xx<FB3A_W;xx++) drow[xx]=g_lut565[srow[xx]];
        }
    }
}
...
static int cmd_load_rom(void)
{
    ...
    /* PicoCartInsert (chemin memoire) attend une ROM DEJA byteswappee : le chemin
       fichier PicoCartLoad fait Byteswap() avant d'appeler PicoCartInsert, et
       PicoCartInsert d/re-byteswappe en interne pour le checksum (cart.c). FAME lit
       les u16 en little-endian hote -> sans ce swap il execute un programme inverse.
       On swappe ICI, APRES le CRC (qui porte sur la ROM big-endian d'origine). */
    /* Le CRC ci-dessus (sur ROM RAW_BE) garantit deja qu'on part d'une ROM brute :
       une ROM deja swappee ne matcherait pas crc_68k. On swappe donc une seule fois. */
    Byteswap((void*)rom, (void*)rom, (int)g_rom_size);
    g_rom_swapped=1; shared[ZZPICO_SH_ROM_STATE]=1;   /* SWAPPED_HOST */
    /* POSTSWAP : reconstruit ce que FAME lira (u16 little-endian hote). Attendu
       SSP=0x00FFFE00, PC=0x00000206. */
    { const u8 *r=rom;
      u32 ssp=((u32)(r[0]|(r[1]<<8))<<16)|(u32)(r[2]|(r[3]<<8));
      u32 pc =((u32)(r[4]|(r[5]<<8))<<16)|(u32)(r[6]|(r[7]<<8));
      shared[ZZPICO_SH_POSTSWAP_SSP]=ssp; shared[ZZPICO_SH_POSTSWAP_PC]=pc; }
...
static void cmd_start(void)
{
    ...
    PicoPower(); PicoReset();
    /* v1.1 (SMS #5) : modes 50/60 explicites selon region, dans les DEUX sens.
       EUR(8)/JAP-PAL(2) -> PAL 50 ; USA(4)/JAPN(1) -> NTSC 60. + PsndRerate. */
    { u32 rgn2=shared[ZZPICO_SH_REGION_IN];
      if((rgn2==8u||rgn2==2u) && !Pico.m.pal){ Pico.m.pal=1; PsndRerate(0); }
      else if((rgn2==4u||rgn2==1u) && Pico.m.pal){ Pico.m.pal=0; PsndRerate(0); }
    }
    ...
    for(;;){
        ...
        if(cmd_seq!=shared[ZZPICO_SH_ACK_SEQ]){
            ...
            {   /* AUTONOME (modele ZZDoom) : une PicoFrame par iteration, pacee par
                ...
                int g_do_render=1;                /* v1.1 (SMS #3a) : gate du DESSIN */
                ...
                g_do_render = 0; PicoIn.skipFrame = 1;
#else
                /* v1.1 (SMS #3a) audio-slaving + decouplage : on ne DESSINE que si le
                   68k a pris la frame precedente (FLIP_SEQ rattrape g_frame_seq).
                   L'emulation + l'audio tournent a CHAQUE iteration (PicoFrame toujours
                   appele ; pace sur le drainage du ring plus bas). skip = DESSIN seul. */
                g_do_render = (shared[ZZPICO_SH_FLIP_SEQ] == g_frame_seq) ? 1 : 0;
                PicoIn.skipFrame = g_do_render ? 0 : 1;
#endif
                ...
                { int pi; for(pi=0;pi<2;pi++){
                    ...
                    if(zp&ZP_C)     pd|=(1u<<5);
                    if(zp&ZP_A)     pd|=(1u<<6);
                    if(zp&ZP_START) pd|=(1u<<7);
                    if(zp&ZP_Z)     pd|=(1u<<8);
                    if(zp&ZP_Y)     pd|=(1u<<9);
                    if(zp&ZP_X)     pd|=(1u<<10);
                    if(zp&ZP_MODE)  pd|=(1u<<11);
                    PicoIn.pad[pi]=pd;
                  }
                }
                ...
                if(g_do_render){   /* v1.1 (SMS #3a) : convert + backbuffer seulement si on dessine */
                    ...
                    if(p96_fast){
                        ...
                        u32 lines=(Pico.video.reg[1]&8)?240u:224u;
                        u32 loffs=(Pico.video.reg[1]&8)?0u:8u;
                        shared[ZZPICO_SH_ACTIVE_H]=lines;
                        shared[ZZPICO_SH_Y_OFFSET]=loffs;
                        /* FAST_CONVERT_LUT : boucle chaude -O3, LUT 565->P96
                           ecrite DIRECTEMENT dans le backbuffer P96. */
                        convert_lut_p96(src, adst, stride_px, loffs, lines);
                ...
                __asm__ volatile("dsb":::"memory");
                /* Frame prete dans le back buffer. On ne PUBLIE que si on a dessine. */
                if(g_do_render) shared[ZZPICO_SH_FRAME_READY] = ++g_frame_seq;
                /* v1.1 (SMS #3a) PACING = audio-slaving. Audio actif -> on se cale sur le
                   DRAINAGE du ring par le 68k (horloge Amiga reelle : AHI comme Paula),
                   PAS sur le compteur ARM (qui derive) NI sur l'affichage. Aucune attente
                   FLIP ici (g_do_render gate deja le rendu sur un buffer pris). Sinon
                   fallback : cadence console CCNT + attente FLIP (comportement V1.0). */
                if(shared[ZZPICO_SH_PCM_ENABLED]){
                    u32 tgt=1400u;          /* paires bufferisees (~63ms) */
                    u32 guard=60000000u;    /* securite anti-freeze */
                    while(guard--){
                        u32 wr=g_pcm_wpos, rd=shared[ZZPICO_SH_PCM_READ_POS];
                        u32 fbq=(wr>=rd)?(wr-rd):(PCM_RING_SIZE-rd+wr);
                        if((fbq>>2) < tgt) break;   /* place pour la frame suivante -> produire */
                    }
                } else {
                    ...
                    g_frame_t0 = pmu_cyc();
                    { u32 s2=8000000u;
                      while(shared[ZZPICO_SH_FLIP_SEQ] != g_frame_seq && s2--){ } }
                }
            }
        }
        shared[ZZPICO_SH_HEARTBEAT]=++hb;
