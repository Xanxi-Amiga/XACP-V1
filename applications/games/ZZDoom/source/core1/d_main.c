...
void doomgeneric_Tick()
{
    ...
    ((volatile unsigned int*)0x04300000UL)[32]++;  /* tick_enter */
    I_StartFrame ();
    TryRunTics (); // will run at least one tic
    ((volatile unsigned int*)0x04300000UL)[33]++;  /* after_TryRunTics */

    S_UpdateSounds (players[consoleplayer].mo);// move positional sounds

    // Update display, next frame, with current state.
    ((volatile unsigned int*)0x04300000UL)[34]++;  /* before_D_Display */
    if (screenvisible)
    {
        ((volatile unsigned int*)0x04300000UL)[35]++;  /* D_Display_enter */
        D_Display ();
    }
}
...
void D_DoomLoop (void)
{
    ((volatile unsigned int*)0x04300000UL)[5]=0xB010UL;
    if (bfgedition &&
    ...
    ((volatile unsigned int*)0x04300000UL)[5]=0xB011UL;
    TryRunTics();
    ((volatile unsigned int*)0x04300000UL)[5]=0xB012UL;

    I_SetWindowTitle(gamedescription);
    I_GraphicsCheckCommandLine();
    I_SetGrabMouseCallback(D_GrabMouseCallback);
    I_InitGraphics();
    ((volatile unsigned int*)0x04300000UL)[5]=0xB013UL;
    I_EnableLoadingDisk();

    V_RestoreBuffer();
    R_ExecuteSetViewSize();

    ((volatile unsigned int*)0x04300000UL)[5]=0xB014UL;
    D_StartGameLoop();
    ...
    ((volatile unsigned int*)0x04300000UL)[5]=0xB015UL;
    /* ZZDoom: loop until 68k sends STOP (shared[3]) */
    while(((volatile unsigned int*)0x04300000UL)[3]==0){
        { extern void zz_audio_pump(void); zz_audio_pump(); }
        doomgeneric_Tick();
        { extern void zz_audio_pump(void); zz_audio_pump(); }
    }
    /* STOP received: stop drawing, disable MMU, idle (Julia WFE rule) */
    { extern void dcache_clean_range(unsigned int,unsigned int);
      extern void mmu_disable(void);
      dcache_clean_range(0x04300000UL, 256);   /* flush shared before MMU off */
      ((volatile unsigned int*)0x04300000UL)[1]=0xFF;
      ((volatile unsigned int*)0x04300000UL)[3]=0;
      mmu_disable(); }
...
void D_DoomMain (void)
{
    ...
    ((volatile unsigned int*)0x04300000UL)[5]=0xB001UL; V_Init ();
    ...
    /* ZZDoom: WAD is in memory, skip filesystem search */
    ((volatile unsigned int*)0x04300000UL)[5]=0xB0A0UL;
    if (((volatile unsigned int *)0x04300000UL)[30] == 1)
    {
        iwadfile = "doom2.wad";
        gamemission = doom2;
        printf("ZZDoom: Doom II IWAD detected\n");
    }
    else
    {
        iwadfile = "DOOM1.WAD";
    ...
    ((volatile unsigned int*)0x04300000UL)[5]=0xB0A1UL;
    D_AddFile(iwadfile);
    ((volatile unsigned int*)0x04300000UL)[5]=0xB0A2UL;
#if ORIGCODE
    ...
    ((volatile unsigned int*)0x04300000UL)[5]=0xB0A3UL;
    D_IdentifyVersion();
    ((volatile unsigned int*)0x04300000UL)[5]=0xB0A4UL;
    InitGameVersion();
    ((volatile unsigned int*)0x04300000UL)[5]=0xB0A5UL;
...
//    W_PrintDirectory();
    ...
    ((volatile unsigned int*)0x04300000UL)[5]=0xB002UL; R_Init ();

    DEH_printf("\nP_Init: Init Playloop state.\n");
    ((volatile unsigned int*)0x04300000UL)[5]=0xB003UL; P_Init ();

    DEH_printf("S_Init: Setting up sound.\n");
    ((volatile unsigned int*)0x04300000UL)[5]=0xB004UL; S_Init (sfxVolume * 8, musicVolume * 8);

    DEH_printf("D_CheckNetGame: Checking network game status.\n");
    ((volatile unsigned int*)0x04300000UL)[5]=0xB005UL; D_CheckNetGame ();

    PrintGameVersion();

    DEH_printf("HU_Init: Setting up heads up display.\n");
    ((volatile unsigned int*)0x04300000UL)[5]=0xB006UL; HU_Init ();

    DEH_printf("ST_Init: Init status bar.\n");
    ((volatile unsigned int*)0x04300000UL)[5]=0xB007UL; ST_Init ();
