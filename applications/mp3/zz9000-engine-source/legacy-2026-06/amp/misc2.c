...
{
        if (is > 0) return (float)(t_43[is]*a);
        else return (float)(-t_43[-is]*a);
}
...
int sfreq=header->sampling_frequency;
	...
	itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=32;
	 ...
	 */
	                ...
	                int scalefac=itsAmp->scalefac_l[gr][ch][0];

			l=0;sfb=0;
			a=fras_l(sfb,global_gain,scalefac_scale,scalefac,preflag);
	                while (l<36) { /* This is most likely meant to be as it is. Turn off aggressive-loop-optimizations to avoid a warning. */
	                        itsAmp->xr[ch][0][l]=fras2(itsAmp->is[ch][l],a);
	                        if (l==itsAmp->t_l[sfb]) {
					scalefac=itsAmp->scalefac_l[gr][ch][++sfb];
					a=fras_l(sfb,global_gain,scalefac_scale,scalefac,preflag);
				}
	                        l++;
	                }
	 ...
	 */
			sfb=3; 
			window_len=itsAmp->t_s[sfb]-itsAmp->t_s[sfb-1];
	                while (l<itsAmp->non_zero[ch]) {
	                	for (window=0;window<3;window++) {
	                        	int scalefac=itsAmp->scalefac_s[gr][ch][sfb][window];
	                                int subblock_gain=info->subblock_gain[gr][ch][window];
					a=fras_s(global_gain,subblock_gain,scalefac_scale,scalefac);
	                                for (i=0;i<window_len;i++) {
      		                                itsAmp->xr[ch][0][t_reorder[header->ID][sfreq][l]]=fras2(itsAmp->is[ch][l],a);
	                                	l++;
                                       	}
                               	}
                               	sfb++;
			...
			while (l<576) itsAmp->xr[ch][0][t_reorder[header->ID][sfreq][l++]]=0;
		} else {
	/*
	...
	/* long blocks */
		...
		while (l<576) itsAmp->xr[ch][0][l++]=0;
	}
}
 ...
 * stereo stuff ****************************************************************
 */
static int find_isbound(struct Amp *itsAmp,int isbound[3],int gr,struct SIDE_INFO *info,struct AUDIO_HEADER *header)
{
int sfb,window,window_len,ms_flag,tmp,i;
   ...
   if (header->mode_extension==1 || header->mode_extension==3) {
	...
	/* find that isbound!
	 */
		tmp=itsAmp->non_zero[1];
		sfb=0; while ((3*itsAmp->t_s[sfb]+2) < tmp  && sfb < 12) sfb++;
		while ((isbound[0]<0 || isbound[1]<0 || isbound[2]<0) && !(info->mixed_block_flag[gr][0] && sfb<3) && sfb) {
			for (window=0;window<3;window++) {
				if (sfb==0) {
					...
					window_len=itsAmp->t_s[sfb]-itsAmp->t_s[sfb-1];
					tmp=(3*itsAmp->t_s[sfb-1]+2) + (window+1)*window_len;
				}
				if (isbound[window] < 0)
					for (i=0;i<window_len;i++)
						if (itsAmp->is[1][tmp--] != 0) {
							isbound[window]=itsAmp->t_s[sfb]+1; 
							break;
						}
			}
			sfb--;
	 ...
	 */
		if (sfb==2 && info->mixed_block_flag[gr][0]) {
			if (isbound[0]<0 && isbound[1]<0 && isbound[2]<0) {
				tmp=35;
				while (itsAmp->is[1][tmp] == 0) tmp--;
				sfb=0; while (itsAmp->t_l[sfb] < tmp  && sfb < 21) sfb++;
				isbound[0]=isbound[1]=isbound[2]=itsAmp->t_l[sfb]+1;
	 ...
	 */
		tmp=itsAmp->non_zero[0];
		sfb=0; while ((3*itsAmp->t_s[sfb]+2) < tmp && sfb < 12) sfb++;
		itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=(itsAmp->t_s[sfb]-1)/6+1; 
	} else {
	 ...
	 */
                ...
                sfb=0; while (itsAmp->t_l[sfb] < tmp && sfb < 21) sfb++;
		isbound[0]=isbound[1]=isbound[2]=itsAmp->t_l[sfb]+1;
		itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=(itsAmp->non_zero[0]-1)/18+1; 
	}
    ...
    */
	...
	if (!info->window_switching_flag[gr][0] || (info->window_switching_flag[gr][0] && info->block_type[gr][0]!=2)) 
		isbound[0]=isbound[1]=isbound[2]=(MAX(itsAmp->non_zero[0],itsAmp->non_zero[1]));
	else isbound[0]=isbound[1]=isbound[2]=576;

	if (info->window_switching_flag[gr][0] && info->block_type[gr][0]==2) {
		/* should do for mixed blocks too, though i havent tested... */
			tmp=(MAX(itsAmp->non_zero[0],itsAmp->non_zero[1]))/3;
			sfb=0; while (itsAmp->t_s[sfb]<tmp && sfb<12) sfb++;
			itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=(itsAmp->t_s[sfb]-1)/6+1;
	}
	else itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=(isbound[0]-1)/18+1;
...
static inline void stereo_s(struct Amp *itsAmp,int l,float a[2],int pos,int ms_flag,int is_pos,struct AUDIO_HEADER *header)
{
float ftmp,Mi,Si;
	...
	if (l>=576) return; /* brrr... */
        ...
        if ((is_pos != IS_ILLEGAL) && (header->ID==1)) {
                ftmp=fras2(itsAmp->is[0][l],a[0]);
                itsAmp->xr[0][0][pos]=(1-t_is[is_pos])*ftmp;
                itsAmp->xr[1][0][pos]=t_is[is_pos]*ftmp;
		return;
	}
	...
	if ((is_pos != IS_ILLEGAL) && (header->ID==0)) {
		ftmp=fras2(itsAmp->is[0][l],a[0]);
		if (is_pos&1) {
			itsAmp->xr[0][0][pos]= t_is2[itsAmp->intensity_scale][(is_pos+1)>>1] * ftmp;
			itsAmp->xr[1][0][pos]= ftmp;
		} else {
			itsAmp->xr[0][0][pos]= ftmp;
			itsAmp->xr[1][0][pos]= t_is2[itsAmp->intensity_scale][is_pos>>1] * ftmp;
		}
		return;
	}

        if (ms_flag) {
                Mi=fras2(itsAmp->is[0][l],a[0]);
                Si=fras2(itsAmp->is[1][l],a[1]);
                itsAmp->xr[0][0][pos]=(Mi+Si)*i_sq2;
                itsAmp->xr[1][0][pos]=(Mi-Si)*i_sq2;
        } else {
                itsAmp->xr[0][0][pos]=fras2(itsAmp->is[0][l],a[0]);
                itsAmp->xr[1][0][pos]=fras2(itsAmp->is[1][l],a[1]);
        }
}

static inline void stereo_l(struct Amp *itsAmp,int l,float a[2],int ms_flag,int is_pos,struct AUDIO_HEADER *header)
{
float ftmp,Mi,Si;
	if (l>=576) return;
...
*/
	if (is_pos != IS_ILLEGAL) {
		ftmp = fras2(itsAmp->is[0][l], a[0]);
		if (header -> ID ==1) {
			itsAmp->xr[0][0][l] = (1 - t_is[is_pos]) * ftmp;
			itsAmp->xr[1][0][l] = t_is[is_pos] * ftmp;
			return;
		} else if (is_pos & 1) {
			itsAmp->xr[0][0][l] = t_is2[itsAmp->intensity_scale][(is_pos + 1) >> 1] * ftmp;
			itsAmp->xr[1][0][l] = ftmp;
		} else {
			itsAmp->xr[0][0][l] = ftmp;
			itsAmp->xr[1][0][l] = t_is2[itsAmp->intensity_scale][is_pos >> 1] * ftmp;
		}
		return;
	}
	...
	if (ms_flag) {
		Mi=fras2(itsAmp->is[0][l],a[0]);
		Si=fras2(itsAmp->is[1][l],a[1]);
		itsAmp->xr[0][0][l]=(Mi+Si)*i_sq2;
		itsAmp->xr[1][0][l]=(Mi-Si)*i_sq2;
	} else {
		itsAmp->xr[0][0][l]=fras2(itsAmp->is[0][l],a[0]);
		itsAmp->xr[1][0][l]=fras2(itsAmp->is[1][l],a[1]);
	}
...
scalefac_scale=info->scalefac_scale[gr];
	 ...
	 */
			...
			ms_flag=find_isbound(itsAmp,isbound,gr,info,header);

			sfb=0; l=0;
			for (ch=0;ch<2;ch++) {
				scalefac[ch]=itsAmp->scalefac_l[gr][ch][0];
			...
			while (l<36) {
				...
				if (l<isbound[0]) is_pos=IS_ILLEGAL;
					...
					/* MPEG2 */
						...
						if (is_pos==itsAmp->is_max[sfb]) is_pos=IS_ILLEGAL;
					}
				}

				stereo_l(itsAmp,l,a,ms_flag,is_pos,header);

				if (l==itsAmp->t_l[sfb]) {
					sfb++;
					for (ch=0;ch<2;ch++) {
						scalefac[ch]=itsAmp->scalefac_l[gr][ch][sfb];
	 ...
	 */
                        ...
                        while (l<(MAX(itsAmp->non_zero[0],itsAmp->non_zero[1]))) {
                                for (window=0;window<3;window++) {
                                        subblock_gain[0]=info->subblock_gain[gr][0][window];
					subblock_gain[1]=info->subblock_gain[gr][1][window];
					scalefac[0]=itsAmp->scalefac_s[gr][0][sfb][window];
					scalefac[1]=itsAmp->scalefac_s[gr][1][sfb][window];

					if (itsAmp->t_s[sfb] < isbound[window]) {
						is_pos=IS_ILLEGAL;
						...
						/* MPEG2 */
							...
							if (is_pos==itsAmp->is_max[sfb+6]) is_pos=IS_ILLEGAL;
						}
						...
						a[0]=fras_s(global_gain[0],subblock_gain[0],scalefac_scale[0],scalefac[0]);
                                        ...
                                        for (i=0;i<window_len;i++) {
                                                stereo_s(itsAmp,l,a,t_reorder[id][sfreq][l],ms_flag,is_pos,header);
                                                l++;
                                        }
                                }
                                sfb++;
				...
				itsAmp->xr[0][0][reorder]=itsAmp->xr[1][0][reorder]=0;
			}
                } else {                                                                       
        /*
	 ...
	 */
			...
			window_len=itsAmp->t_s[0]+1;
				 ...
				 * window_len=-itsAmp->t_s[sfb]+itsAmp->t_s[++sfb];
				 */
				window_len  = -itsAmp->t_s[sfb++];
				window_len +=  itsAmp->t_s[sfb];
	 ...
	 */
		...
		sfb=0; l=0;
		scalefac[0]=itsAmp->scalefac_l[gr][0][sfb];
		a[0]=fras_l(sfb,global_gain[0],scalefac_scale[0],scalefac[0],preflag[0]);
		scalefac[1]=itsAmp->scalefac_l[gr][1][sfb];
...
static int min_cycles = 99999999;
        ...
        __asm__(".byte 0x0f,0x31" : "=a" (cnt1), "=d" (cnt4));
			...
			{
				register float Mi = fras2(itsAmp->is[0][l],a[0]);
				register float Si = fras2(itsAmp->is[1][l],a[1]);
				register float tmp = i_sq2;
				itsAmp->xr[0][0][l]=(Mi+Si)*tmp;
				itsAmp->xr[1][0][l]=(Mi-Si)*tmp;
			}
 ...
 * requantize_downmix **********************************************************
 */
void requantize_downmix(struct Amp *itsAmp, int gr, struct SIDE_INFO *info, struct AUDIO_HEADER *header)
{
int l,sfb,ms_flag,i;
...
float a[2];
	...
	*/
	if (info->window_switching_flag[gr][0] && info->block_type[gr][0]==2)
		if (info->mixed_block_flag[gr][0]) {
			// downmix for mixed blocks is not yet implemented
			return;
		} else {
			int window,window_len;
			int isbound[3];
			...
			sfb=0; l=0; window_len=itsAmp->t_s[0]+1;
			...
			while (l<(MAX(itsAmp->non_zero[0],itsAmp->non_zero[1]))) {
				for (window=0;window<3;window++) {
					...
					subblock_gain[1]=info->subblock_gain[gr][1][window];
					scalefac[0]=itsAmp->scalefac_s[gr][0][sfb][window];
					is_pos=scalefac[1]=itsAmp->scalefac_s[gr][1][sfb][window];
					...
					if (itsAmp->t_s[sfb] < isbound[window]) {
						...
						if (ms_flag) {
							for (i=0;i<window_len;i++) {
								register float Mi=fras2(itsAmp->is[0][l],a[0]);
								itsAmp->xr[0][0][t_reorder[id][sfreq][l]]=Mi*i_sq2;
								l++;
							}
						} else {
							a[1]=fras_s(global_gain[1],subblock_gain[1],scalefac_scale[1],scalefac[1]);
							for (i=0;i<window_len;i++) {
								register float tmp1=fras2(itsAmp->is[0][l],a[0]);
								register float tmp2=fras2(itsAmp->is[1][l],a[1]);
								itsAmp->xr[0][0][t_reorder[id][sfreq][l]]=(tmp1+tmp2)*0.5f;
								l++;
							}
						}
					} else {
						a[0]=fras_s(global_gain[0],subblock_gain[0],scalefac_scale[0],scalefac[0]);
						for (i=0;i<window_len;i++) {
							register float ftmp = fras2(itsAmp->is[0][l], a[0]);
							if (id==0 && is_pos<itsAmp->is_max[sfb])
								ftmp*=t_downmix[itsAmp->intensity_scale][(is_pos+1)>>1];
							itsAmp->xr[0][0][t_reorder[id][sfreq][l]] = ftmp;
							l++;
						}						
					}
				}
		...
		if (header->mode_extension==1 || header->mode_extension==3) {
			int tmp=itsAmp->non_zero[1];
			while (itsAmp->is[1][tmp] == 0) tmp--;
			sfb=0; while (itsAmp->t_l[sfb] < tmp && sfb < 21) sfb++;
			isbound=itsAmp->t_l[sfb]+1;
			itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=(itsAmp->non_zero[0]-1)/18+1;
		} else {
			isbound=(MAX(itsAmp->non_zero[0],itsAmp->non_zero[1]));
			itsAmp->no_of_imdcts[0]=itsAmp->no_of_imdcts[1]=(isbound-1)/18+1;
		}
		...
		if (ms_flag)
			...
			while (l < isbound) {
				register float tmp1=fras2(itsAmp->is[0][l],a[0]);
				register float tmp2=fras2(itsAmp->is[1][l],a[1]);
				itsAmp->xr[0][0][l]=(tmp1+tmp2)*0.5f;
	                        if (l==itsAmp->t_l[sfb]) {
        	                        sfb++;
                	                scalefac[0]=itsAmp->scalefac_l[gr][0][sfb];
                        	        a[0]=fras_l(sfb,global_gain[0],scalefac_scale[0],scalefac[0],preflag[0]);
