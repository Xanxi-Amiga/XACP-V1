...
ULONG __stdargs L_OpenLibs(struct EngineBase *EngineBase);
void  __stdargs L_CloseLibs(void);
...
const char __aligned EnLibName [] __used = LIBNAME;
const char __aligned EnLibID   [] __used = LIBNAME" "NUM2STR(VERSION)"."NUM2STR(REVISION)" ("NUM2STR(DATE)")\0";
const char __aligned VERSTRING [] __used = "\0$VER: " LIBNAME".library "NUM2STR(VERSION)"."NUM2STR(REVISION)" ("NUM2STR(DATE)")\0";
...
{
	0xe000,8,NT_LIBRARY,
	0x0080,10,(ULONG) &EnLibName[0],
	0xe000,14,LIBF_SUMUSED|LIBF_CHANGED,
	0xd000,20,VERSION,
	0xd000,22,REVISION,
	0x80,24,(ULONG) &EnLibID[0],
	(ULONG) 0
};
#else
{
	INITBYTE(OFFSET(Node,		   ln_Type),	  NT_LIBRARY),
	0x80, (UBYTE) OFFSET(Node,     ln_Name),	  (ULONG) &EnLibName[0],
	INITBYTE(OFFSET(Library,	   lib_Flags),	  LIBF_SUMUSED|LIBF_CHANGED),
	INITWORD(OFFSET(Library,	   lib_Version),  VERSION),
	INITWORD(OFFSET(Library,	   lib_Revision), REVISION),
	0x80, (UBYTE) OFFSET(Library,  lib_IdString), (ULONG) &EnLibID[0],
	(ULONG) 0
};
#endif
...
ULONG __stdargs L_OpenLibs(struct EngineBase *EngineBase) {
	...
	EngineBase->eb_DOSBase = DOSBase;

	return iEngine_OpenLibs();
}
